...
def peut_placer(bateaux, grille, id_bat, pos, direction):
    ...
    if (x < 0) or (y < 0): return False # on vérifie que les coordonées sont dans la grille

    if (direction == "+y") and (y + taille_bat <= N):
       if not np.any(grille[x, y:y+taille_bat]): return True # on vérifie que les cases de la grille sont libres

    elif (direction == "+x") and (x + taille_bat <= N):
...
def main():
    print (nb_pos(boat, np.zeros((10,10)) ,1))
    print(len(sys.argv))
